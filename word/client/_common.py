...
t1_requests = ["examples", "definitions", "relatedWords"]
t2_requests = ["randomWord"]


def get_template_string(template: Template, **kwargs):
    word = kwargs.pop("word", None)
    suffix = kwargs.pop("suffix", None)
    if word is not None:
        template = Template(template.safe_substitute(word=word))
    if suffix is not None:
        template = Template(template.safe_substitute(suffix=suffix))
    return template.safe_substitute()


def get_api_url(api_protocol, api_host, api_endpoint, api_key):
    return f"{api_protocol}{api_host}{api_endpoint}?api_key={api_key}"
